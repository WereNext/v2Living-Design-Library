...
import { generateColorPalette, extractThemeColors } from './color-utils';

export function mapTokensToChakra(theme: Theme): ThemeConfig & { colors: Record<string, unknown>; fonts: Record<string, string>; radii: Record<string, string>; space: Record<string, string>; shadows: Record<string, string> } {
  const c = extractThemeColors(theme.colors);
  const primary = theme.colors.primary || '221.2 83.2% 53.3%';
  const secondary = theme.colors.secondary || '210 40% 96%';
